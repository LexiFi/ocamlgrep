...
let search ~make_valid_source_path ~before ~after query_exprs cmt =
  ...
  List.filter_map
    (fun ({ loc_start; loc_end; loc_ghost } as loc : Location.t) ->
      if loc_ghost then None
        ...
        let lines_before =
          let b = max 1 (s - before) in
          List.init (s - b) (fun k -> src_lines.(b - 1 + k))
        in
        let lines_after =
          let a = min num_lines (e + after) in
          List.init (a - e) (fun k -> src_lines.(e + k))
        in
        Some
          { Export.location = location_of_loc loc source_path;
            lines_before; lines; lines_after })
